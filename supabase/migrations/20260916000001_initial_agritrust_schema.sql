-- ==============================================================================
-- AgriTrustGeoAgent - Production Database Migration
...
CREATE POLICY "Agronomists can view escalated fields"
    ...
    USING (
        ...
        EXISTS (
            ...
            WHERE escalation_tickets.field_id = fields.id
        )
    );
...
CREATE POLICY "Agronomists can view evidence in escalations"
    ...
    USING (
        public.current_user_role() = 'agronomist' AND
        EXISTS (
            SELECT 1 FROM public.diagnostic_inferences di
            JOIN public.escalation_tickets et ON et.inference_id = di.id
            WHERE di.evidence_id = crop_evidence.id
        )
    );
...
CREATE POLICY "Growers view own diagnostic inferences"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM public.crop_evidence ce
            WHERE ce.id = diagnostic_inferences.evidence_id AND ce.uploader_id = auth.uid()
        )
    );
